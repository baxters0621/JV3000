...
[2026-09-11 13:36] · Módulo: Herramientas de validación · Cambio: Corregida la expresión regular de `tools/validar.ps1` que analizaba la configuración de `DB_NAME`; PowerShell estaba interpolando `$jv_config_value` dentro de una cadena con comillas dobles y provocaba una excepción antes de completar la validación. · Archivos: tools/validar.ps1, docs/BITACORA.md · Prueba: ejecución del validador completo después del ajuste.
[2026-09-11 13:38] · Módulo: Despliegue / Recursos · Cambio: Corregida la ruta relativa de las fuentes Bootstrap Icons para que el navegador encuentre `bootstrap-icons.woff2` desde `assets/css/bootstrap-icons.css`; ajustado además el validador para leer el fallback real de `JV_DB_NAME` en `backup.bat`. · Archivos: assets/css/bootstrap-icons.css, tools/validar.ps1, docs/BITACORA.md · Prueba: comprobación HTTP de los assets y ejecución del validador.
[2026-09-11 13:40] · Módulo: Dashboard · Cambio: Corregido el endpoint de actualización AJAX para conservar la ruta MVC `url=dashboard`; antes el front controller redirigía la solicitud sin `url` y el navegador la reportaba erróneamente como sesión vencida. · Archivos: assets/dashboard/index.js, docs/BITACORA.md · Prueba: node --check y prueba funcional del dashboard autenticado.
[2026-09-11 14:15] · Módulo: Despliegue / Permisos · Cambio: Verificada la instalación mediante Apache real: los directorios internos responden 403, los assets públicos responden 200 y el validador completo finaliza correctamente. Revisadas las ACL de roles en código; los usuarios de prueba Operador y Operador_Ventas permanecen inactivos y pendientes, por lo que no se modificaron datos para forzar una prueba de login. · Archivos: .htaccess, includes/Security.php, includes/sidebar.php, controllers/ProductosController.php, controllers/ComprasController.php, docs/BITACORA.md · Prueba: `tools/validar.ps1 -BaseUrl http://localhost/JV3000_db -Database jv3000_db` = `[OK] Validacion completada`; consulta de usuarios y roles.
[2026-09-11 14:32] · Módulo: Control de acceso · Cambio: Prueba funcional reversible de los roles Operador de Carga y Operador de Ventas mediante activación temporal; Carga accedió a Inventario y Compras y fue rechazado en Ventas, mientras Ventas accedió a Salidas y Estadísticas y no mostró Compras. Se restauraron exactamente los estados, hashes y credenciales originales de ambos usuarios. · Archivos: docs/BITACORA.md · Prueba: login y navegación en Apache real; verificación SQL posterior de `status=Inactivo`, `aprobado=0` y hashes originales.

[2026-09-02 12:00] · Módulo: Categorías / Compras · Cambio: Fix bug categorías no creaba + tabla compras esquinada. A) Categorías: `catAbrirForm()` llamaba `addEventListener`/`removeEventListener` sobre la instancia de Bootstrap Modal (`modalCategorias`) en vez del elemento DOM (`modalCategoriasEl`). Las instancias Modal no heredan de EventTarget — fix: usar `modalCategoriasEl` para listeners. B) Compras: `.card-jv-table` tenía `overflow: visible` que permitía a las filas flotantes (border-spacing:10px + border-radius:14px) sobrepasar las esquinas redondeadas del contenedor — fix: `overflow: hidden`. · Archivos: assets/modules/productos/productos.js, assets/modules/compras/compras.css · Prueba: node --check JS sin errores; fix de 2 líneas por archivo.

[2026-09-02 12:15] · Módulo: Categorías/UI · Cambio: Modal de formulario de categorías con tamaños de fuente más grandes para mejor legibilidad. Overrides específicos para `#modalCat`: chip 32→40px, título sección .88→1.05rem, subtítulo .75→.88rem, label .82→.92rem, input .85→.95rem, hint .75→.82rem, padding sección 14→18px, footer 14→16px, max-width 540px. · Archivos: assets/modules/productos/productos.css · Prueba: CSS sin errores de sintaxis.
...
---
[2026-09-11 12:45] · Módulo: Seguridad e instalación · Cambio: Se incorporaron dependencias Bootstrap locales, se endureció la serialización JSON de datos dinámicos, se corrigió el respaldo con contraseña, se alineó la BD predeterminada a jv3000_db y se reparó la codificación del seed v5 · Archivos: assets/css/bootstrap.min.css, assets/css/bootstrap-icons.css, assets/js/bootstrap.bundle.min.js, views/layouts/main.php, views/salidas/index.php, views/compras/index.php, includes/config.php, backups/backup.bat, db/jv3000_portable_v5.sql, docs/README.md, AGENTS.md, .gitignore · Prueba: php -l correcto en 63 archivos; assets locales existentes; seed sin literales ??; diff sin errores de whitespace

[2026-09-11 12:55] · Módulo: Seguridad y navegación · Cambio: Se escaparon los textos dinámicos del detalle de salidas, se eliminó la redirección al dashboard legado y se habilitó el grupo de reportes para los roles permitidos por la operación · Archivos: assets/modules/salidas/salidas.js, index.php, includes/sidebar.php · Prueba: Sintaxis PHP validada y revisión de sinks HTML completada

[2026-09-11 13:10] · Módulo: Sesiones y AJAX · Cambio: Se eliminó el cierre de sesión por pestañas duplicadas, se convirtió el dashboard legado en puente MVC y se añadieron comprobaciones de estado HTTP/sesión vencida a los refrescos AJAX · Archivos: assets/js/diseno.js, includes/diseno.php, dashboard/index.php, assets/dashboard/index.js, assets/modules/estadisticas/estadisticas.js · Prueba: Validación sintáctica PHP y revisión de respuestas AJAX completadas

[2026-09-11 13:20] · Módulo: Validación y mantenimiento · Cambio: Se actualizó el validador para usar el seed v5 y la configuración actual de base de datos, evitando falsos avisos por la migración de configuración · Archivos: tools/validar.ps1 · Prueba: Sintaxis PHP/JavaScript y diff verificados

[2026-09-11 13:30] · Módulo: Documentación y operación · Cambio: Se ampliaron las guías con configuración de entorno, actualización segura, respaldo/restauración, validación, puesta en producción, continuidad y checklist diario · Archivos: docs/README.md, docs/MANUAL_USUARIO.md · Prueba: referencias comprobadas contra config/.env.example, backup.bat, configurar_backup.bat y tools/validar.ps1

[2026-09-11 13:35] · Módulo: Documentación · Cambio: Se corrigió la numeración de las nuevas secciones operativas del manual para evitar encabezados duplicados · Archivos: docs/MANUAL_USUARIO.md · Prueba: revisión de encabezados completada

[2026-09-11 13:25] · Módulo: Respaldos y continuidad · Cambio: Se corrigió el script de backup para ejecutarse sin errores en Windows, usar MYSQL_PWD sin exponer la clave en argumentos y conservar limpieza automática de archivos con más de 30 días · Archivos: backups/backup.bat · Prueba: backup real generado y restaurado en jv3000_restore_test_20260911; 22 tablas y 3 roles validados; base temporal eliminada

[2026-09-11 13:28] · Módulo: Seguridad · Cambio: Se reemplazó addslashes por serialización JSON segura en el handler de eliminación del catálogo de Compras · Archivos: views/compras/index.php · Prueba: revisión de handlers inline y sintaxis PHP/JavaScript validadas

[2026-09-11 13:40] · Módulo: Limpieza estructural · Cambio: Se eliminaron los dashboards PHP duplicados de perfil y usuarios sin referencias activas; se conservaron sus assets porque los controladores MVC los utilizan y se mantuvo el puente dashboard/index.php · Archivos: dashboard/perfil.php, dashboard/usuarios.php · Prueba: búsqueda recursiva de referencias y validación sintáctica posteriores

[2026-09-11 13:50] · Módulo: Base de datos · Cambio: Se separaron las migraciones DDL del arranque HTTP y se creó tools/migrar.php con ejecución CLI idempotente y tabla schema_migrations · Archivos: init.php, tools/migrar.php · Prueba: validación sintáctica y ejecución pendiente

[2026-09-11 13:55] · Módulo: Base de datos y documentación · Cambio: Se retiró el marcador de pestaña obsoleto del bootstrap y se documentó la ejecución explícita de tools/migrar.php para instalaciones existentes · Archivos: init.php, docs/README.md, docs/MANUAL_USUARIO.md · Prueba: migración idempotente ejecutada dos veces; versión registrada y 23 tablas verificadas
