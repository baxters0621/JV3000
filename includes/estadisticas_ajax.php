...
$compras_7d = number_format($db->fetchOne("SELECT COALESCE(SUM(cantidad * precio_costo), 0) as total FROM compras WHERE fecha_compra >= DATE_SUB(CURRENT_DATE, INTERVAL 6 DAY) AND status = 'Activa'")['total'], 2);

$margen_7d_raw = (float)$db->fetchOne("SELECT COALESCE(SUM(s.cantidad * (s.precio_venta - p.precio_costo)), 0) as margen FROM salidas s JOIN productos p ON s.id_producto = p.id_producto WHERE s.fecha_salida >= DATE_SUB(CURRENT_DATE, INTERVAL 6 DAY) AND s.id_tipo_mov = 1 AND s.status = 'Activa'")['margen'];
$margen_7d = number_format($margen_7d_raw, 2);

$costo_vendido_7d = number_format($db->fetchOne("SELECT COALESCE(SUM(s.cantidad * p.precio_costo), 0) as total FROM salidas s JOIN productos p ON s.id_producto = p.id_producto WHERE s.fecha_salida >= DATE_SUB(CURRENT_DATE, INTERVAL 6 DAY) AND s.id_tipo_mov = 1 AND s.status = 'Activa'")['total'], 2);
